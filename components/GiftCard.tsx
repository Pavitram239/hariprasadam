...
export default function GiftCard({ giftBox }: GiftCardProps) {
  const giftDisplayName = giftBox.title.toLowerCase().startsWith('gift box')
    ? giftBox.title
    : `Gift Box ${giftBox.code ? giftBox.code + ' - ' : ''}${giftBox.title}`;
  ...
  return (
    <div className="bg-white border border-[#E6DEC8] rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col justify-between group">
        ...
        <div className="p-6">
          <div className="flex items-baseline justify-between gap-2 border-b border-[#F2ECE1] pb-3 mb-3">
            ...
            <div className="text-right shrink-0">
              ...
              <span className="font-serif text-2xl font-bold text-[#A85A2A]">
          ...
          {/* Curated Jars List */}
          {giftBox.curatedItems && giftBox.curatedItems.length > 0 && (
          ...
          {/* Presentation Note */}
          {giftBox.presentation && giftBox.presentation.length > 0 && (
      ...
      <div className="p-6 pt-0">
        <WhatsAppButton
          type="gift"
          targetName={giftDisplayName}
          label="Enquire About This Gift"
          variant="secondary"
          size="md"
